...
export default function App() {
  ...
  const setThreadTitle = (threadId, title) => {
    setThreads((prev) =>
      prev.map((t) =>
        t.id === threadId ? { ...t, title: title.slice(0, 40) } : t,
      ),
    );
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch("https://velox-backend-6sqt.onrender.com/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            {threads.map((t) => (
              <div
                ...
                style={{
                  ...
                  ...(hoveredId === t.id && t.id !== activeId
                    ? styles.historyItemHover
                    : {}),
                }}
              >
                {t.title}
